...
setTimeout(() => {
	for (let b = 0; b < rowLength; b++) {
		...
		newImg.src=foxArray[b]['thumbnail image'];
		newImg.setAttribute('id', ('Fox' + b));
		console.log(foxArray[b]['thumbnail image']);
		
		//empty cell and id generation
		let tabCell = genTable.rows[(1+b)].insertCell(-1);
		tabCell.setAttribute('id', ('Fox' + b));
		
		
		//append children images to each new cell element
		document.getElementById('Fox' + b).appendChild(newImg);
...
//console.log(foxArray[1]['thumbnail image']);
...
setTimeout(() => {
	for (let b = 0; b < rowLength; b++) {
		...
		console.log(rowLength);
		let tabCell = genTable.rows[(1+b)].insertCell(-1);
		tabCell.innerHTML = foxArray[b]['thumbnail image'];
		//document.createElement('table').insertRow(-1).insertCell(-1).innerHTML = foxArray[b]['thumbnail image'];		
	}
